...
def Zubov_training(col_points:jax.Array,
    ...
    def Zubov_data_loss(model:nnx.Module):
        x_zeros=jnp.zeros(2)
        COLLOC_MODEL,GRAD_MODEL= model.value_and_gradient(col_points)
        COLLOC_SYS=system(col_points,args)
        COLLOC_OMEGA=omega(col_points)
        ZEROS_MODEL=model(x_zeros)
        DATA_MODEL=model(data_points[0])
...
optimizer1=nnx.Optimizer(model1, optax.adamw(learning_rate=1e-3))
args=(5.0,)
# args=(0.125,)
#%%
...
optimizer3=nnx.Optimizer(model3, optax.adamw(learning_rate=1e-3))
data_points=RESULTS
